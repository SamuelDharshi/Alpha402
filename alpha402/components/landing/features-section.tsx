...
const features = [
  {
    ...
    description: "Parses your natural language intent using Groq AI. Collapses complex strategies into structured, actionable blueprints.",
    visual: "ai",
  },
  {
    number: "02",
    title: "Intel Agent",
    description: "Monitors real-time price feeds and high-fidelity data. Logs every micro-signal on 0G Storage for persistent audit trails.",
    visual: "deploy",
  },
  {
    number: "03",
    title: "Risk Agent",
    description: "Evaluates trade scores across the AXL mesh. Real-time reasoning using decentralized compute nodes for maximum integrity.",
    visual: "security",
  },
  {
    number: "04",
    title: "Execution Agent",
    description: "Submits transactions via KeeperHub for guaranteed execution. Private routing and gas optimization without human intervention.",
    visual: "collab",
  },
];
  ...
  return (
    <svg viewBox="0 0 200 160" className="w-full h-full">
      ...
      {[0, 1, 2, 3, 4, 5].map((i) => {
        ...
        const radius = 50;
        const x2 = Number((100 + Math.cos(angle) * radius).toFixed(8));
        const y2 = Number((80 + Math.sin(angle) * radius).toFixed(8));
        return (
          <g key={i}>
            {/* Connection line */}
            <line
