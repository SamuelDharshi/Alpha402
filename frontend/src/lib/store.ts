...
export const useAlphaStore = create<AlphaStoreState>((set, get) => ({
  ...
  addBeam: (sender, receiver) => {
    ...
    setTimeout(() => get().removeBeam(id), 1500);
  },
  ...
  connect: () => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'A2A_MESSAGE' && msg.data) {
          const raw = msg.data;
          const sender = raw.from || 'commander';
          const receiver = raw.to || 'user';
          ...
          setTimeout(() => get().updateAgentStatus(sender, 'IDLE'), 2000);
        }

        if (msg.type === 'HISTORY' && Array.isArray(msg.data)) {
          msg.data.forEach((raw: Record<string, unknown>) => {
            const transformed: A2AMessage = {
              id: String(raw.id || Math.random().toString()),
              sender: String(raw.from || 'commander'),
              receiver: String(raw.to || 'user'),
    ...
    ws.onerror = (err) => {
      console.error('[Alpha402] WS error:', err);
    };
  },
