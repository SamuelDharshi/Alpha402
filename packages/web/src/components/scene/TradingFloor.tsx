  ...
  return (
    <div className="w-full h-full relative">
      <Canvas shadows dpr={[1, 2]}>
        ...
        <OrthographicCamera makeDefault position={[10, 10, 10]} zoom={miniaturized ? 60 : 80} />
        <OrbitControls enablePan={false} enableZoom={!miniaturized} autoRotate={!miniaturized} autoRotateSpeed={0.5} maxPolarAngle={Math.PI / 2.1} minPolarAngle={Math.PI / 4} />
        ...
        {activeBeams.map(beam => <MessageBeam key={beam.id} sender={beam.sender} receiver={beam.receiver} />)}
        {bursts.map(burst => <ParticleBurst key={burst.id} position={burst.pos} />)}
      </Canvas>
    </div>
  );
};
